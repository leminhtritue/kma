...
def train_source(args):
    ...
    args.modelpath = args.output_dir_src + '/source_F1.pt'   
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset=='VISDA-C':
                ...
                # log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name_src, iter_num, max_iter, acc_s_te)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy source (train/test/target) = {:.2f}% / {:.2f}% / {:.2f}%, Loss = {:.2f}'.format(args.name_src, \
                	iter_num, max_iter, acc_s_tr, acc_s_te, acc_s_tgt, total_loss/count_loss)
            total_loss = 0.0
            count_loss = 0
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if acc_s_te >= acc_init:
                ...
                best_netB = netB.state_dict()
                best_netC = netC.state_dict()
                torch.save(best_netF, osp.join(args.output_dir_src, "source_F1.pt"))
                torch.save(best_netB, osp.join(args.output_dir_src, "source_B1.pt"))
                torch.save(best_netC, osp.join(args.output_dir_src, "source_C1.pt"))
            netF.train()
            netB.train()
            netC.train()
