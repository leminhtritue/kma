...
def cal_accWH(loader, netF, netB, netC, netBRF, netCRF):
    ...
    all_label_eq = all_label[mask_HeqRF]
    softmax_output_h_eq = softmax_output_h[mask_HeqRF]
    softmax_output_rf_eq = softmax_output_rf[mask_HeqRF]
    ...
    print("Acc total H (refined by RF): {:.2f}%, Acc total RF (refined by H): {:.2f}%\n".format(accuracy_h_refined*100, accuracy_rf_refined*100))
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        entropy_si_loss = torch.mean(loss.Entropy(softmax_si))
        msoftmax_si = softmax_si.mean(dim=0)
        gentropy_si_loss = torch.sum(-msoftmax_si * torch.log(msoftmax_si + args.epsilon))
        entropy_si_loss -= gentropy_si_loss
        im_si_loss = entropy_si_loss * args.ent_par
        classifier_loss += im_si_loss
