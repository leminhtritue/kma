...
def train_source(args):
    ...
    netBRF = network.feat_bootleneck_rf(nrf=args.nf, typrf, type=args.classifier, gamma = args.gamma, bottleneck_dim=args.bottleneck).cuda()
    netCRF = network.feat_classifier_rf(nrf=args.nre=args.layer_rf, class_num = args.class_num).cuda()
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%/{:.2f}%/{:.2f}%'.format(args.dset, iter_num, max_iter, acc_s_tr, acc_s_te, acc_t_te)
